...
const FRONTEND_FORBIDDEN = [
  {
    ...
    suggestion: 'Użyj logger.error() z @/lib/logger',
  },
  {
    pattern: /console\.warn\s*\(/g,
    suggestion: 'Użyj logger.warn() z @/lib/logger',
  },
];
...
function getFileCategory(
  ...
  if (
    relativePath.startsWith('supabase/functions/') &&
  ...
  for (const catchBlock of catchBlocks) {
    if (catchBlock.includes('return') && !catchBlock.includes('flush')) {
      warnings.push({
        ...
        pattern: 'await flush()',
        suggestion:
          'Brak flush() przed return w catch block - eventy Sentry mogą nie zostać wysłane',
      });
    }
  }
...
function formatOutput(analyses: FileAnalysis[]): string {
  ...
  // Grupuj po typie
  const frontendAnalyses = withWarnings.filter(
    (a) => a.category.type === 'frontend',
  );
  const edgeFunctionAnalyses = withWarnings.filter(
    (a) => a.category.type === 'edge-function',
  );
